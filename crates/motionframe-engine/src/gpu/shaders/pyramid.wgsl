...
fn main(@builtin(global_invocation_id) id: vec3u) {
    ...
    let c00 = textureLoad(in_tex, vec2(min(sx, iw - 1), min(sy, ih - 1)), 0).r;
    let c10 = textureLoad(in_tex, vec2(min(sx + 1, iw - 1), min(sy, ih - 1)), 0).r;
    let c01 = textureLoad(in_tex, vec2(min(sx, iw - 1), min(sy + 1, ih - 1)), 0).r;
    let c11 = textureLoad(in_tex, vec2(min(sx + 1, iw - 1), min(sy + 1, ih - 1)), 0).r;

    let avg = (c00 + c10 + c01 + c11) * 0.25;
    textureStore(out_tex, vec2(x, y), vec4(avg, 0.0, 0.0, 0.0));
}
